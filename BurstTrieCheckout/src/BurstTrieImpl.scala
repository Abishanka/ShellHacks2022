...
@SerialVersionUID(100L) class BurstTrieImpl[T, S <: Iterable[T]] (burstFactor: Int, growthFactor: Int, accumulateHit: (S, T) => S, emptyHits: () => S) extends BurstTrie[T] with Serializable {
  ...
  private class AccessNode(len: Int = 1) extends Node with Serializable {
    ...
    @SerialVersionUID(1223L)
    class AccessIterator (q: String) extends Iterator[T] with Serializable{
  ...
  @SerialVersionUID(6463L)
  private class ContainerNode(len: Int = 1) extends Node with Serializable{
